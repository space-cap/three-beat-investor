...
def run_real_test():
    """
    .env 파일의 정보를 사용하여 실제 KIS API를 호출하는 테스트를 수행합니다.
    """
    # .env 파일에서 환경 변수 로드
    load_dotenv()
    ...
    try:
        ...
        # 테스트용 조건검색 키 (필요시 변경)
        condition_key = "P000000123"
        
        print(f"\n조건 검색 테스트를 시작합니다 (조건 키: {condition_key})...")
        
        # 조건 검색 API 호출
        stocks = client.fetch_conditional_search(condition_key=condition_key)
